...
docker compose up -d api-facturas
curl.exe http://localhost:8058/     # → "motor":"mariadb"
...
```powershell
curl.exe -i http://localhost:8058/api/factura/999                 # → 404 (SIGNAL 1644 traducido)
...
| `mariadb` no queda healthy | Puerto 13349 ocupado, o volumen de un intento fallido: `docker compose down -v` y de nuevo |
