...
- [ ] Crear el `docker-compose.yml` con el servicio `postgres` (imagen
      16-alpine, volumen `pgdata`, puerto 15450, healthcheck con
...
cliente SQL a `localhost:15450` (usuario `postgres`) ve las **12 tablas**
...
- [ ] `appsettings.json` con la cadena de conexión (default
      `localhost:15450` para correr sin Docker).
...
- [ ] `api_facturas/Dockerfile`: imagen `dotnet/sdk:10.0`, `dotnet watch`,
      `ASPNETCORE_URLS` en 8058, `DOTNET_USE_POLLING_FILE_WATCHER`.
- [ ] Agregar al `docker-compose.yml` el servicio `api-facturas`: `build:`,
      código montado + `bin/` y `obj/` en volúmenes anónimos, puerto 8058,
