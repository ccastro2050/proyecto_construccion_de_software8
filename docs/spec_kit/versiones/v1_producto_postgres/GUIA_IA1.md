...
8. En mi máquina TAMBIÉN corre el proyecto clonado del curso con sus
   ...
   a. Publica los puertos del host con +100: en el docker-compose.yml la
      API va "8158:8058" y PostgreSQL va "15550:1433" (adentro de los
   ...
   Docker) apunta a localhost,15550. Cuando me des URLs o comandos de
   prueba, usa localhost:8158 (API) y localhost,15550 (BD).
...
3. **El punto de control real es el smoke test final** (7_quickstart.md):
   ...
   (regla 8 del prompt) — donde el quickstart diga `localhost:8058` use
   `localhost:8158`, y donde diga `15450` use `15550`.
...
5. En esta máquina TAMBIÉN corre el proyecto clonado del curso. MI proyecto
   publica los puertos del host con +100 (API "8158:8058", PostgreSQL
   "15550:1433") y su docker-compose.yml empieza con `name: mi_v1_producto`.
6. Al final, corre el smoke test completo de 7_quickstart.md §2 (con mis
   puertos: localhost:8158) y muéstrame la evidencia de los 6 criterios de
