...
flowchart TD
    ...
    F1["Fase 1: el esqueleto Flask en el compose<br/>(hola marca, sin login)"] -->|"localhost:8059 responde con la marca"| F2
...
- [ ] Servicio `front-flask` (:8059) en el compose, con
      `API_FACTURAS_URL` y `CLAVE_SESION` por entorno.

**Verificar:** `http://localhost:8059` muestra el header Azul Cordillera
con el logosímbolo.
