...
Flask + Jinja2 (`front_flask/`, puerto **8059**) con la identidad del
...
flowchart LR
    ...
    subgraph F["front_flask :8059 — NUEVO"]
        R["rutas Flask<br/>(vistas)"] --> CA["cliente_api<br/>(HTTP)"]
    end
    subgraph A["api_facturas :8058 — INTACTA (v1-v5)"]
        API["los 51 endpoints"]
    end
    BD[("los TRES motores")]
    NAV -->|"HTML (Jinja2)"| F
    CA -->|"JSON: api-facturas:8058"| API
    API --> BD
    F -.->|"PROHIBIDO"| BD
```
...
**Incluye:** `front_flask/` en el compose (8059) · la marca (base.html +
...
2. **Login:** crear un usuario de prueba por la API → entrar en
   `http://localhost:8059/login` → redirige a productos; con clave mala
