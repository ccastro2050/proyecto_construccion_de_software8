...
WORKDIR /app
ENV DOTNET_USE_POLLING_FILE_WATCHER=1 ASPNETCORE_URLS=http://0.0.0.0:8058
EXPOSE 8058
...
| `EXPOSE 8058` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            FRONTFLASK["SERVIDOR WEB (front)<br/>contenedor front-flask<br/>hostname: front-flask · escucha en 8059"]
            APIFACTURAS["SERVIDOR DE APLICACIONES<br/>contenedor api-facturas<br/>hostname: api-facturas · escucha en 8058"]
    ...
    NAV -->|"localhost:8058"| APIFACTURAS
    NAV -->|"localhost:8059"| FRONTFLASK
    ...
    FRONTFLASK -->|"http://api-facturas:8058<br/>al controlador, por el NOMBRE"| APIFACTURAS
    SQLSERVERINIT -->|"espera el healthcheck,<br/>siembra y termina"| SQLSERVER
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:13349"| MARIADB
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:15450"| POSTGRES
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:11449"| SQLSERVER
```
...
```yaml
  postgres:
    ...
    ports:
      - "15450:5432"                 # "puerto en su PC : puerto interno"
...
1. **Dos redes de nombres**: hacia su PC, puertos publicados
   (`localhost:8058`, `localhost:15450`); entre contenedores, nombres de
...
docker network create proyecto_construccion_de_software8_default

# 2. postgres
docker run -d --name postgres --network proyecto_construccion_de_software8_default --restart unless-stopped `
  -e "POSTGRES_PASSWORD=Construccion123!" `
  ...
  -p 15450:5432 postgres:16-alpine

# 3. ESPERAR a que responda de verdad… mirándolo a ojo

# 4. sqlserver
docker run -d --name sqlserver --network proyecto_construccion_de_software8_default --restart unless-stopped `
  -e "ACCEPT_EULA=Y" `
  -e "MSSQL_SA_PASSWORD=Construccion123!" `
  -e "MSSQL_PID=Developer" `
  -v mssqldata:/var/opt/mssql `
  -p 11449:1433 mcr.microsoft.com/mssql/server:2022-latest

# 5. ESPERAR a que responda de verdad… mirándolo a ojo

# 6. sqlserver-init
docker run -d --name sqlserver-init --network proyecto_construccion_de_software8_default --restart no --entrypoint /bin/bash `
  -e "MSSQL_SA_PASSWORD=Construccion123!" `
  -v "${PWD}/db:/scripts:ro" mcr.microsoft.com/mssql/server:2022-latest /scripts/init_sqlserver.sh

# 7. mariadb
docker run -d --name mariadb --network proyecto_construccion_de_software8_default --restart unless-stopped `
  -e "MARIADB_ROOT_PASSWORD=Construccion123!" `
  -v mariadbdata:/var/lib/mysql `
  -v "${PWD}/db/bdfacturas_mariadb.sql:/docker-entrypoint-initdb.d/bdfacturas_mariadb.sql:ro" `
  -p 13349:3306 mariadb:11
...
docker run -d --name api-facturas --network proyecto_construccion_de_software8_default --restart unless-stopped `
  -e "ConnectionStrings__Postgres=Host=postgres;Port=5432;Database=bdfacturas_postgres_local;Username=postgres;Password=Construccion123!" `
  -e "ConnectionStrings__SqlServer=Server=sqlserver,1433;Database=bdfacturas_sqlserver_local;User Id=sa;Password=Construccion123!;TrustServerCertificate=True;" `
  -e "ConnectionStrings__MariaDb=Server=mariadb;Port=3306;Database=bdfacturas_mariadb_local;User ID=root;Password=Construccion123!;AllowUserVariables=True" `
  -e "Motor=${MOTOR_BD:-postgres}" `
  -v "${PWD}/api_facturas:/app" `
  -v /app/bin `
  -v /app/obj `
  -p 8058:8058 api-facturas

# 10. Construir la imagen de front-flask y encenderla
docker build -t front-flask ./front_flask
docker run -d --name front-flask --network proyecto_construccion_de_software8_default --restart unless-stopped `
  -e "API_FACTURAS_URL=http://api-facturas:8058" `
  -e "CLAVE_SESION=Construccion123!Sesion" `
  -v "${PWD}/front_flask:/app" `
  -p 8059:8059 front-flask
...
```yaml
URL_API: http://api-facturas:8058
```
...
|---|---|---|
| **Su navegador** | `http://localhost:8058` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-facturas:8058` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8058:8058`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8058`, el front
...
```powershell
docker network inspect proyecto_construccion_de_software8_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_construccion_de_software8` produce
la red `proyecto_construccion_de_software8_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_construccion_de_software8-front-flask-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_construccion_de_software8_default has active endpoints (name:"proyecto_construccion_de_software8-front-flask-1" …)
```
...
> contenedor lo alcanza: desde `front-flask` de este proyecto,
...
```bash
# ¿«front-flask» alcanza la base de SU propio proyecto? Le pregunta por el nombre
...
docker compose exec front-flask getent hosts postgres
...
docker compose exec front-flask curl -s -o /dev/null -w "%{http_code}\n" \
