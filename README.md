...
> ## 🌐 ENTRAR AL FRONT → **http://localhost:8059**
...
> el puerto 8058 (pasa al reiniciar el PC: la API vieja revive sin su
...
> ```
> NAMES                                               STATUS                    PORTS
> proyecto_construccion_de_software8-api-facturas-1   Up 2 hours                0.0.0.0:8058->8058/tcp
> proyecto_construccion_de_software8-postgres-1       Up 2 hours (healthy)      0.0.0.0:15450->5432/tcp
> ```
>
...
```powershell
git clone https://github.com/ccastro2050/proyecto_construccion_de_software8.git
cd proyecto_construccion_de_software8
...
| Qué | Dónde |
|---|---|
| **API Facturas** — diagnóstico | http://localhost:8058/ |
| **Swagger** (documentación interactiva: ver y probar los endpoints) | http://localhost:8058/swagger |
| **FRONT** (v8) — la aplicación completa, con facturación | http://localhost:8059 |
| **Entrar al front — usuario ADMINISTRADOR** | `demo@correo.com` / `Demo123!` |
| Crearlo (una sola vez, 2 comandos) | `curl.exe -X POST http://localhost:8058/api/usuario -H "Content-Type: application/json" -d "{\"email\":\"demo@correo.com\",\"contrasena\":\"Demo123!\"}"` y luego `curl.exe -X POST http://localhost:8058/api/rol-usuario -H "Content-Type: application/json" -d "{\"fkemail\":\"demo@correo.com\",\"fkidrol\":1}"` (el segundo le da el rol Administrador: sin él, el menú sale recortado) |
| Listar productos | http://localhost:8058/api/producto |
| PostgreSQL (para SQLTools/pgAdmin, opcional) | `localhost:15450` · `postgres`/`Construccion123!` |
| SQL Server (opcional — v4) | `localhost,11449` · `sa`/`Construccion123!` |
| MariaDB (opcional — v5) | `localhost:13349` · `root`/`Construccion123!` |
...
> ℹ️ Este proyecto usa los puertos 8058 y 15450: si alguno ya está ocupado
...
```
proyecto_construccion_de_software8/
...
│
├── api_facturas/                # LA API DE LA v1 — C#/ASP.NET Core (puerto 8058)
...
│   ├── appsettings.json         # Cadena de conexión (default localhost:15450)
